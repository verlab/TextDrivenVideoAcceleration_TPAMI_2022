...
def download_val_set(num_workers, failed_log, compress, verbose, skip, log_file):
  """
  Download the test set.
  :param num_workers:           Number of downloads in parallel.
  :param failed_log:            Where to save failed video ids.
  ...
  with open(config.VAL_METADATA_PATH) as file:
    data = json.load(file)

  pool = parallel.Pool(None, data, config.VALID_ROOT, num_workers, failed_log, compress, verbose, skip,
                       log_file=log_file)
  pool.start_workers()
  pool.feed_videos()
  pool.stop_workers()
...
def main(args):
  ...
  if args.all:
    ...
    if args.val:
      # download the val set
      download_val_set(args.num_workers, args.failed_log, args.compress, args.verbose, args.skip, args.failed_log)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--val", action="store_true", help="download the val set")
